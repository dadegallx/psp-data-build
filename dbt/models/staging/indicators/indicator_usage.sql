...
with organization_indicator_usage as (
    select
        ...
        ssi.short_name as indicator_short_name,
        ssi.dimension,
        count(distinct ss.id) as surveys_using_indicator,
        max(sd.created_date_ms / 1000) as last_measurement_timestamp
    ...
    inner join {{ source('data_collect', 'survey_stoplight_indicator') }} ssi
        on ss.survey_stoplight_indicator_id = ssi.id
    group by
        sd.organization_id,
        ssi.id,
        ssi.code_name,
...
family_counts as (
    ...
    inner join {{ source('data_collect', 'snapshot_stoplight') }} sns
        ...
        and ss.id = sns.survey_stoplight_id
    group by
        sd.organization_id,
        ssi.id
),
